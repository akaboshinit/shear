...
> Dart & Flutter プロジェクトの未使用ファイル、依存関係、エクスポート、メンバーを検出。
...
- **未宣言依存関係検出** — コード内でインポートされているが `pubspec.yaml` に宣言されていないパッケージを検出
- **未使用エクスポート検出** — 他のファイルからインポートされていないエクスポートシンボル（クラス、関数など）を発見（conditional import 対応）
- **未使用 enum メンバー検出** — プロジェクト内で参照されていない enum 定数を検出
- **未使用クラスメンバー検出** — 参照されていないクラス/mixin のパブリックメンバーを検出（名前ベースのヒューリスティック）
...
| オプション | 説明 | デフォルト |
|---|---|---|
| `--include <type>` | 削除対象のカテゴリ: `files`, `dependencies`, `exports`, `enumMembers`, `classMembers` | すべて |
...
| オプション | 説明 | デフォルト |
|---|---|---|
| `--include <type>` | 検出する問題の種類: `files`, `dependencies`, `exports`, `enumMembers`, `classMembers` | すべて |
...
- **カテゴリカード** — タイプ別の内訳（ファイル、依存関係、未宣言依存関係、エクスポート、enum メンバー、クラスメンバー）
...
コード内で**インポートされている**が `pubspec.yaml` に**宣言されていない**パッケージを検出します。

- 重大度：**error**（デフォルト）
- 実際の `package:` import と `pubspec.yaml` の宣言を比較
- プロジェクト自身のパッケージ名は無視
- `ignoreDependencies` 設定を考慮
...
プロジェクト内で**参照されていない** enum 定数を検出します。

- 重大度：**warning**（デフォルト）
- 全ファイルの `EnumName.member` プレフィックス付き参照を追跡
- `EnumName.values` が参照されている場合、全メンバーを使用済みとみなす
- part ファイルを正しく処理（ライブラリとその part から enum を収集）

### 未使用クラスメンバー

**参照されていない**クラス/mixin のパブリックメンバー（メソッド、静的フィールド、名前付きコンストラクタ）を検出します。

- 重大度：**warning**（デフォルト）
- **名前ベースのヒューリスティック**（型解決なし）：
  - 静的メンバーは `ClassName.member` のプレフィックス付き参照で追跡
  - インスタンスメンバーは全ファイルの名前ベースの参照で追跡
- `@override` メソッドとオペレーターは除外
- プライベートメンバー（`_` で始まるもの）は除外
- ⚠️ 無関係な型間の名前衝突により偽陽性が発生する可能性あり
...
| `unlistedDetect` | 未宣言依存関係の検出 |
| `exportDetect` | 未使用エクスポートの検出 |
| `enumDetect` | 未使用 enum メンバーの検出 |
| `classDetect` | 未使用クラスメンバーの検出 |
...
└──────────┘    └─────────────────────┘    │ (依存関係)     │
                           ...
                           ▼            ▼            ▼            ▼            ▼            ▼
                      ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐
                      │ 未使用   │ │ 未使用   │ │ 未宣言   │ │ 未使用   │ │ 未使用   │ │ 未使用   │
                      │ファイル  │ │依存関係  │ │依存関係  │ │エクスポート│ │ enum    │ │クラス   │
                      │ 検出    │ │ 検出     │ │ 検出    │ │ 検出     │ │メンバー  │ │メンバー  │
                      └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘
                           │           │           │           │           │           │
                           └─────┬─────┘───────────┘───────────┘───────────┘───────────┘
                                      │
...
4. **検出** — 6 つの検出器がグラフに対して独立に実行され、未使用コードを検出
